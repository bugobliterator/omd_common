...
bool flash_journal_write_from_2_buffers(struct flash_journal_instance_s* instance, size_t entry_buf1_size, const void* entry_buf1, size_t entry_buf2_size, const void* entry_buf2) {
    ...
    struct flash_journal_entry_s* new_entry_ptr;
    ...
    bool success = true;
    uint8_t* write_ptr = (uint8_t*)new_entry_ptr;
    success = success && flash_write(write_ptr, sizeof(entry_header), &entry_header);
    write_ptr += sizeof(entry_header);

    if (entry_buf1) {
        success = success && flash_write(write_ptr, entry_buf1_size, entry_buf1);
        write_ptr += entry_buf1_size;
    }

    if (entry_buf2) {
        success = success && flash_write(write_ptr, entry_buf2_size, entry_buf2);
        write_ptr += entry_buf2_size;
    }
...
static bool flash_journal_entry_valid(struct flash_journal_instance_s* instance, const struct flash_journal_entry_s* entry) {
    ...
    return flash_journal_range_in_bounds(instance, entry, entry_size) && flash_journal_entry_compute_crc16(entry) == entry->crc16;
}
...
static size_t flash_journal_entry_size(uint8_t len) {
    return FLASH_JOURNAL_ENTRY_HEADER_SIZE + len;
}
